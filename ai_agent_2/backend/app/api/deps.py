...
async def get_current_app_admin(
    ...
    """Get current user and verify the global ``is_app_admin`` flag.

    Distinct from ``CurrentAdmin`` (which checks the per-org ADMIN role) —
    this gate guards platform-level routes (e.g. ``/admin/users``) that should
    be reachable only by deployment operators, not org-scoped admins.
    """
    if not getattr(current_user, "is_app_admin", False):
...
CurrentAppAdmin = Annotated[User, Depends(get_current_app_admin)]
