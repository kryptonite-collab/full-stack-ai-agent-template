...
```bash
cd backend
uv run ai_agent_2 <group> <command> [options]
```

### Server Commands

```bash
uv run ai_agent_2 server run              # Start dev server
uv run ai_agent_2 server run --reload     # With hot reload
uv run ai_agent_2 server run --port 9000  # Custom port
uv run ai_agent_2 server routes           # Show all registered routes
```

### Database Commands

```bash
uv run ai_agent_2 db init                  # Run all migrations
uv run ai_agent_2 db migrate -m "message"  # Create new migration
uv run ai_agent_2 db upgrade               # Apply pending migrations
uv run ai_agent_2 db upgrade --revision e3f  # Upgrade to specific revision
uv run ai_agent_2 db downgrade             # Rollback last migration
uv run ai_agent_2 db downgrade --revision base  # Rollback to start
uv run ai_agent_2 db current               # Show current revision
uv run ai_agent_2 db history               # Show migration history
```
...
uv run ai_agent_2 user create

# Create user non-interactively
uv run ai_agent_2 user create --email user@example.com --password secret

# Create user with specific role
uv run ai_agent_2 user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run ai_agent_2 user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run ai_agent_2 user create-admin --email admin@example.com --password secret

# Change user role
uv run ai_agent_2 user set-role user@example.com --role admin

# List all users
uv run ai_agent_2 user list
```

### Celery Commands

```bash
uv run ai_agent_2 celery worker                    # Start worker
uv run ai_agent_2 celery worker --loglevel debug   # Debug logging
uv run ai_agent_2 celery worker --concurrency 8    # 8 worker processes
uv run ai_agent_2 celery beat                      # Start scheduler
uv run ai_agent_2 celery flower                    # Start Flower UI
uv run ai_agent_2 celery flower --port 5556        # Custom Flower port
```
...
```bash
uv run ai_agent_2 cmd <command-name> [options]
```
...
uv run ai_agent_2 cmd rag-ingest ./docs/guide.pdf

# Ingest a directory
uv run ai_agent_2 cmd rag-ingest ./docs/

# Ingest recursively into a specific collection
uv run ai_agent_2 cmd rag-ingest ./docs/ --collection knowledge --recursive

# Ingest with sync mode
uv run ai_agent_2 cmd rag-ingest ./docs/ --sync-mode new_only
uv run ai_agent_2 cmd rag-ingest ./docs/ --sync-mode update_only

# Skip replacing existing documents
uv run ai_agent_2 cmd rag-ingest ./docs/ --no-replace
```
...
uv run ai_agent_2 cmd rag-search "what is fastapi"

# Search a specific collection
uv run ai_agent_2 cmd rag-search "deployment guide" --collection docs

# Get more results
uv run ai_agent_2 cmd rag-search "deployment" --top-k 10
```
...
uv run ai_agent_2 cmd rag-collections

# Show overall RAG system statistics
uv run ai_agent_2 cmd rag-stats

# Drop a collection (with confirmation)
uv run ai_agent_2 cmd rag-drop my_collection

# Drop without confirmation
uv run ai_agent_2 cmd rag-drop my_collection --yes
```
...
uv run ai_agent_2 cmd rag-sources

# Add a new sync source
uv run ai_agent_2 cmd rag-source-add \
    --name "My Drive" \
    --type gdrive \
    --collection docs \
...
uv run ai_agent_2 cmd rag-source-remove <source-id>
uv run ai_agent_2 cmd rag-source-remove <source-id> --yes  # Skip confirmation

# Trigger sync for a specific source
uv run ai_agent_2 cmd rag-source-sync <source-id>

# Trigger sync for all active sources
uv run ai_agent_2 cmd rag-source-sync --all
```
...
```bash
uv run ai_agent_2 cmd my-command --name test
```
