...
{%- endif %}
    ...
    async def register_webhook(self, bot_id: str) -> dict[str, Any]:
        """Register a webhook URL with the bot's platform.

        DB lookups stay sync; the platform call is async (network I/O).
        """
        bot = self.get(bot_id)
        adapter = get_adapter(bot.platform)
        token = self.get_decrypted_token(bot)
    ...
    async def delete_webhook(self, bot_id: str) -> dict[str, Any]:
        """Remove the webhook from the bot's platform."""
        bot = self.get(bot_id)
        adapter = get_adapter(bot.platform)
        token = self.get_decrypted_token(bot)
        success = await adapter.delete_webhook(token)
