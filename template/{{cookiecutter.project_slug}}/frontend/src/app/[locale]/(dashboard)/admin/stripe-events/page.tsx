...
export default function StripeEventsPage() {
  ...
  const handleReplay = async (evt: StripeEvent) => {
    if (usingStub) {
      toast.info(
        "Demo mode — backend wiring required (POST /admin/stripe-events/{id}/replay)",
      );
      return;
    }
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative w-full max-w-xs">
          <Search className="text-foreground/45 absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2" />
          <Input
        ...
        <div className="flex gap-1">
          {STATUS_FILTER_OPTIONS.map((s) => (
            <button
              ...
              className={cn(
                "border-foreground/15 inline-flex rounded-full border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider transition-colors",
                statusFilter === s
      ...
      <div className="border-foreground/10 bg-card overflow-hidden rounded-2xl border">
        <div className="border-foreground/10 text-foreground/55 hidden grid-cols-[24px_1.4fr_1fr_0.8fr_0.5fr_0.5fr_80px] items-center gap-3 border-b px-5 py-3 font-mono text-[10px] uppercase tracking-wider sm:grid">
          <span />
          <span>Event</span>
          <span>Customer</span>
          <span>Amount</span>
            ...
            {filtered.map((e) => (
              <li key={e.id}>
                ...
                >
                  ...
                  <div className="text-foreground/55 hidden font-mono text-[11px] uppercase tracking-wider sm:block">
                  ...
                  <div className="border-foreground/10 bg-foreground/[0.02] border-t px-5 py-4">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2">
                      ...
                      <a
                        ...
                        className="text-foreground/65 hover:text-foreground inline-flex items-center gap-1 font-mono text-[11px] uppercase tracking-wider"
                      >
                        Open in Stripe →
                      </a>
                    </div>
...
function StatusPill({
  ...
  return (
    ...
    >
      <span aria-hidden className="bg-current h-1 w-1 rounded-full" />
      {status}
      {attempts > 1 && ` · ${attempts}×`}
    </span>
  );
...
function KV({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <dt className="text-foreground/45 font-mono text-[10px] uppercase tracking-wider">
        {label}
      </dt>
      <dd
        className={cn(
