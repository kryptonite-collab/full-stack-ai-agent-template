        ...
        <div className="space-y-10">
          ...
          {hero && (
            ...
            >
              ...
              <div
                aria-hidden
                className="bg-brand/[0.08] pointer-events-none absolute -right-32 -top-32 h-80 w-80 rounded-full blur-[120px]"
              />
              <div
                aria-hidden
                className="bg-foreground/[0.04] pointer-events-none absolute -left-32 -bottom-32 h-72 w-72 rounded-full blur-[120px]"
              />

              <div className="relative flex flex-col gap-4">
                <span className="text-foreground/55 inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-wider">
                  <span aria-hidden className="bg-brand h-1 w-1 rounded-full" />
                  {t("featured")}
                  <span aria-hidden className="text-foreground/30">·</span>
                  {formatDate(hero.date, locale)}
                </span>
                <h2 className="font-display text-foreground text-3xl font-bold leading-tight tracking-tight md:text-4xl">
                  {hero.title}
                </h2>
                <p className="text-foreground/70 text-base leading-relaxed">{hero.excerpt}</p>
                <div className="text-foreground/55 mt-1 flex flex-wrap items-center gap-3 font-mono text-[11px] uppercase tracking-wider">
              ...
              <div className="border-foreground/10 bg-foreground/[0.02] relative hidden flex-col justify-between overflow-hidden rounded-2xl border p-6 md:flex">
                  ...
                  {hero.tags && hero.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {hero.tags.map((tag) => (
                        <span
                          key={tag}
                          className="border-foreground/15 text-foreground/65 inline-flex rounded-full border px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider"
                        >
                          {tag}
                        </span>
                      ))}
                ...
                <div
                  aria-hidden
                  className="font-display text-foreground/8 absolute -right-6 -bottom-12 select-none text-[14rem] font-black leading-none tracking-tighter"
                >
              ...
              {rest.map((p) => (
                <li key={p.slug}>
                  ...
                  >
                    ...
                    {p.tags && p.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {p.tags.slice(0, 2).map((tag) => (
                          <span
                            key={tag}
                            className="border-foreground/10 text-foreground/55 inline-flex rounded-full border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider"
                          >
                            {tag}
                          </span>
                        ))}
                    ...
                    <h3 className="text-foreground font-display text-xl font-bold leading-tight tracking-tight">
                      {p.title}
                    </h3>
                    ...
                    <div className="border-foreground/10 mt-1 flex items-center justify-between border-t pt-3">
                      <span className="text-foreground/55 font-mono text-[10px] uppercase tracking-wider">
