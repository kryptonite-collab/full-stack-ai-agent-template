...
export function ApiKeyManager() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setKeys((prev) => [
        { ...created, token: undefined as unknown as string } as ApiKey,
        ...prev,
      ]);
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-destructive hover:text-destructive shrink-0 h-8 rounded-full"
                  >
                    <Trash2 className="h-3.5 w-3.5" />
                    <span className="ml-1.5 sr-only sm:not-sr-only">Revoke</span>
                  </Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Any service using this key will stop working immediately. You can&apos;t
                      undo this.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="key-name" className="text-foreground/80 text-xs font-medium uppercase tracking-wider">
              Name
            </Label>
            <Input
              id="key-name"
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <div className="border-foreground/10 bg-foreground/[0.04] flex items-center gap-2 rounded-xl border p-3">
              <code className="text-foreground flex-1 break-all font-mono text-xs">
                {showFull ? revealedToken : revealedToken.replace(/(.{6}).+/, "$1" + "•".repeat(40))}
              </code>
              <button
                type="button"
