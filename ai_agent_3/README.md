...
└── commands/             # Click CLI commands (auto-discovered by `ai_agent_3 cmd …`)
...
The generated project ships a Click CLI exposed as `ai_agent_3` (after `make install`):

```bash
ai_agent_3 server run --reload          # dev server
ai_agent_3 db upgrade                   # apply migrations
ai_agent_3 db migrate -m "message"      # create new migration
ai_agent_3 user create-admin            # interactive admin creation
ai_agent_3 rag-ingest <path> -c docs    # ingest local files
ai_agent_3 rag-search "query" -c docs   # semantic search
ai_agent_3 rag-collections              # list collections
ai_agent_3 celery worker                # start worker
ai_agent_3 celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `ai_agent_3 --help` for full CLI docs.
...
ai_agent_3 rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from Google Drive (service-account auth)
ai_agent_3 rag-sync-gdrive --collection documents --folder-id <id>

# Semantic search
ai_agent_3 rag-search "your query" --collection documents
```
