...
Create Date: 2026-05-09T12:40:00+00:00

Creates tables that earlier migrations (0005, 0006, 0007, 0009) modify but
never created. Splits feature-flagged groups so the schema only contains
what was selected at generation time.
"""
...
def upgrade() -> None:
    ...
    op.create_table(
        "tool_calls",
        sa.Column("id", PG_UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("message_id", PG_UUID(as_uuid=True), sa.ForeignKey("messages.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id", PG_UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("conversation_id", PG_UUID(as_uuid=True), sa.ForeignKey("conversations.id", ondelete="CASCADE"), nullable=False),
        sa.Column("shared_by", PG_UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False),
        sa.Column("shared_with", PG_UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("message_id", PG_UUID(as_uuid=True), sa.ForeignKey("messages.id", ondelete="CASCADE"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("collection_name", sa.String(255), nullable=False),
        sa.Column("sync_source_id", PG_UUID(as_uuid=True), sa.ForeignKey("sync_sources.id", ondelete="SET NULL"), nullable=True),
        sa.Column("status", sa.String(20), nullable=False),
    ...
    op.create_index("ix_channel_identities_platform_user_id", "channel_identities", ["platform_user_id"])
    op.create_index("ix_channel_identities_user_id", "channel_identities", ["user_id"])

    op.create_table(
        "channel_sessions",
        sa.Column("id", PG_UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("bot_id", PG_UUID(as_uuid=True), sa.ForeignKey("channel_bots.id", ondelete="CASCADE"), nullable=False),
        sa.Column("identity_id", PG_UUID(as_uuid=True), sa.ForeignKey("channel_identities.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_channel_sessions_platform_chat_id", "channel_sessions", ["platform_chat_id"])
