"""Business logic for user-scoped slash command settings.
...
class UserSlashCommandService:
    ...
    async def create_custom(
        ...
        try:
            ...
            # Race: another request inserted the same name between our check
            # and the flush. Surface as 409, not 500.
            raise AlreadyExistsError(
                message="Slash command with this name already exists",
                details={"name": data.name},
            ) from exc
    ...
    async def upsert_builtin_override(
        ...
        if existing.prompt is not None:
            # User has a CUSTOM command with the same name — refuse the
            # override; frontend should never trigger this combination.
    ...
    async def update(
        ...
        # Renames need to check uniqueness AND the row's role:
        #   - Built-in overrides can't be renamed (the name is the link to
        #     the in-code command).
        ...
        if "prompt" in update_data and db_cmd.prompt is None:
            # Caller is trying to add a prompt to a built-in override row,
            # which would convert it into a custom command. Refuse — they
            # should delete + create instead so the semantic boundary stays
            # clear.
            raise BadRequestError(
