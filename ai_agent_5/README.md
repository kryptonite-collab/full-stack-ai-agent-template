...
└── commands/             # Click CLI commands (auto-discovered by `ai_agent_5 cmd …`)
...
The generated project ships a Click CLI exposed as `ai_agent_5` (after `make install`):

```bash
ai_agent_5 server run --reload          # dev server
ai_agent_5 db upgrade                   # apply migrations
ai_agent_5 db migrate -m "message"      # create new migration
ai_agent_5 user create-admin            # interactive admin creation
ai_agent_5 rag-ingest <path> -c docs    # ingest local files
ai_agent_5 rag-search "query" -c docs   # semantic search
ai_agent_5 rag-collections              # list collections
ai_agent_5 celery worker                # start worker
ai_agent_5 celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `ai_agent_5 --help` for full CLI docs.
...
ai_agent_5 rag-ingest /path/to/docs/ --collection documents --recursive

# Semantic search
ai_agent_5 rag-search "your query" --collection documents
```
