...
```bash
# Local files
uv run ai_agent rag-ingest /path/to/docs/ --collection documents --recursive
```

### Search

```bash
uv run ai_agent rag-search "your query" --collection documents
```

### Manage collections

```bash
uv run ai_agent rag-collections   # List all
uv run ai_agent rag-stats          # Show stats
uv run ai_agent rag-drop <name>    # Delete collection
```
...
```bash
uv run ai_agent cmd rag-sources          # List configured sources
uv run ai_agent cmd rag-source-add       # Add a new source
uv run ai_agent cmd rag-source-sync      # Trigger sync for a source
uv run ai_agent cmd rag-source-remove    # Remove a source
```
