...
**llm_quality_eval** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, SQLite (sync)
, JWT + API Key auth, PydanticAI, RAG (chromadb)
...
# RAG
uv run llm_quality_eval rag-collections
uv run llm_quality_eval rag-ingest /path/to/file.pdf --collection docs
uv run llm_quality_eval rag-search "query" --collection docs

# Sync Sources
uv run llm_quality_eval cmd rag-sources
uv run llm_quality_eval cmd rag-source-add
uv run llm_quality_eval cmd rag-source-sync
```
