...
class ChromaVectorStore(BaseVectorStore):
    ...
    async def insert_document(self, collection_name: str, document: Document) -> None:
        ...
        metadatas = [
            self._build_chunk_metadata(chunk, document) for chunk in document.chunked_pages
        ]
    ...
    async def get_collection_info(self, collection_name: str) -> CollectionInfo:
        ...
        return CollectionInfo(
            name=collection_name, total_vectors=count, dim=self.settings.embeddings_config.dim
        )
